...
export class OperationHelpers {
   ...
   */
  constructor({ logger, logComponent }: { logger: any; logComponent: any }) {
    this.logger = logger;
   ...
   */
  logOperationStep(operationName: string, stage: string, message: string, metadata: Record<string, any> = {}) {
    const stageMap = {
   ...
   */
  createNoOpCleanupHandler(operationName: string, resultProperty: string | null = null) {
   ...
   */
  validateOperationResult(result: Record<string, any>, fieldName: string, operationName: string) {
    if (!result[fieldName]) {
   ...
   */
  getOperationResult(context: Record<string, any>, operationName: string) {
    return context.results.find((r: Record<string, any>) => r.name === operationName);
  }
}
