  ...
  startEventSubscriptions() {
    ...
    try {
      ...
      const unsubscribe = this.subscribeToBlockchainEvents(eventFilter, (event) => {
        ...
        if (event.type) {
          if (event.type.includes('SpreadsheetCreated')) eventType = 'SpreadsheetCreated';else
          if (event.type.includes('VersionSaved')) eventType = 'VersionSaved';else
          if (event.type.includes('CellLocked')) eventType = 'CellLocked';else
          if (event.type.includes('CellUnlocked')) eventType = 'CellUnlocked';
        }
