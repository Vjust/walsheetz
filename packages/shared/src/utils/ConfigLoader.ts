...
import { transactionExperienceManager } from "./helpers/TransactionExperience.js";

interface WalrusConfig {
  aggregatorUrl: string;
  aggregatorFallback?: string;
  publisherUrl: string;
  publisherFallback?: string;
  maxRetries: number;
  retryDelay: number;
  features?: { epochsDefault?: number };
}
...
interface AppConfig {
  ...
  switchNetwork?: (name: string) => Promise<NetworkConfig>;
  getFeature?: (path: string, defaultValue?: unknown) => unknown;
  setFeature?: (path: string, value: unknown) => void;
  getProxyUrl?: (service: string) => string | null;
  getServiceUrl?: (service: string, path?: string) => string;
  getWalrusServiceBase?: (service: string) => string;
  getWalrusFallback?: (service: string) => string;
  getSuiRpcFallback?: () => string;
  resolveHealthyServiceUrl?: (service: string, path?: string, opts?: Record<string, unknown>) => Promise<string>;
}
...
interface ABI {
  modules: Record<string, { name: string; functions: string[]; structs: string[] }>;
...
class ConfigLoader {
  ...
  private loadingPromise: Promise<AppConfig> | null;
  private abiCache: Map<string, ABICacheEntry>;
  private networkValidationCache: Map<string, CacheEntry<ValidationResult>>;
  private isFallback: boolean;
  private fallbackAttemptCount: number;
  ...
  // Validate config structure
  private _validateConfig(config: AppConfig): void {
  ...
  private _detectCurrentNetwork(config: AppConfig): string {
  ...
  private _addRuntimeMethods(config: AppConfig): void {
    const self = this;
    ...
    config.getFeature = function (featurePath, defaultValue = false) {
      ...
      let current: Record<string, unknown> = this.features;
    ...
    config.resolveHealthyServiceUrl = async function (service, path = '/v1/api', opts: Record<string, unknown> = {}) {
      ...
      const tryFetch = async (u: string) => {
        ...
        const timeoutMs = (opts.timeout as number) || 8000;
        ...
        try {
          ...
          if (opts.suppressErrors !== false && (err.name === 'AbortError' || err.message.includes('404'))) {
  ...
  private _getFallbackConfig(): AppConfig {
    ...
    const fallbackConfig: AppConfig = {
      ...
      timestamp: new Date().toISOString(),
      currentNetwork: 'testnet', // Will be overwritten below
  ...
  async validateNetwork(networkName: string): Promise<ValidationResult> {
  ...
  async detectABI(packageId: string, networkName: string | null = null): Promise<ABI> {
    const config = await this.getConfig();
  ...
  private _extractABIFromModules(modules: Record<string, { exposedFunctions?: Record<string, Record<string, unknown>>; structs?: Record<string, Record<string, unknown>> }>): ABI {
    const abi: ABI = {
      modules: {},
      functions: {},
      structs: {}
    ...
    Object.entries(modules).forEach(([moduleName, moduleData]) => {
      ...
      Object.entries(moduleData.exposedFunctions || {}).forEach(([funcName, funcData]) => {
        ...
        abi.functions[fullName] = {
          module: moduleName,
          name: funcName,
          visibility: funcData.visibility as string,
          isEntry: (funcData.isEntry as boolean) || false,
          parameters: (funcData.parameters as unknown[]) || [],
          returnType: (funcData.return_ as unknown[]) || [],
          typeParameters: (funcData.typeParameters as unknown[]) || []
        };
      });
      ...
      Object.entries(moduleData.structs || {}).forEach(([structName, structData]) => {
        ...
        abi.structs[fullName] = {
          module: moduleName,
          name: structName,
          abilities: (structData.abilities as string[]) || [],
          fields: (structData.fields as unknown[]) || [],
          typeParameters: (structData.typeParameters as unknown[]) || []
        };
      });
    });
  ...
  checkFunctionCompatibility(abi: ABI, moduleName: string, functionName: string, expectedParams: unknown[] | null): { compatible: boolean; reason?: string; expected?: unknown[]; actual?: unknown[]; function?: ABIFunction } {
  ...
  persistForcedFeature(featurePath: string, value: unknown): void {
    try {
      if (typeof window !== 'undefined' && typeof localStorage !== 'undefined') {
  ...
  getForcedFeature(featurePath: string): unknown {
    try {
      if (typeof window !== 'undefined' && typeof localStorage !== 'undefined') {
        const key = `walsheetz_forced_feature_${featurePath}`;
