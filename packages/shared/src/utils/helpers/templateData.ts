...
interface TemplateConfig {
  celldata?: CellData[];
  a1Cells?: Record<string, CellValue>;
  rows: number;
  cols: number;
  sheets: SheetConfig[];
  description: string;
...
function convertA1ToCoords(a1: string): { r: number; c: number } | null {
...
function convertTemplateToCelldata(a1Cells: Record<string, CellValue>): CellData[] {
  ...
  for (const [a1, cellValue] of Object.entries(a1Cells)) {
    ...
    const v = cellValue.v !== undefined ? cellValue.v : '';
...
export function getTemplateData(template: string): TemplateConfig {
