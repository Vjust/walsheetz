...
import {
  ...
  LogComponent,
  configLoader,
  eventBus as transactionEventBus,
} from '@dreamlit/walrus';
// Import transaction and offline services from walrus-sui-core
import {
  transactionManager,
  offlineModeService,
} from '@dreamlit/walrus-sui-core/transaction-management';

// ErrorCategory stub (not exported from walrus)
const ErrorCategory = {
  NETWORK: 'network',
  WALLET: 'wallet',
  CONTRACT: 'contract',
...
const progressiveEnhancementService = {
  ...
  executeWithFallback: async <T>(primary: () => Promise<T>, fallback: () => Promise<T>): Promise<T> => {
    try {
  ...
  getServiceStatus: () => ({ healthy: true, degradationLevel: 0, availableServices: ['walrus', 'blockchain'] }),
...
const validationGuards = {
  runPreflightChecks: async () => ({ passed: true, warnings: [] }),
};

const ErrorFactory = {
  fromError: (originalError: any) => {
    const error: any = new Error(originalError.message || 'Unknown error');
...
export class BlockchainAdapter extends IBlockchainService {
  ...
  constructor(storageAdapter = null) {
    ...
    // Initialize services (only in browser environment, not in tests)
    if (
    ...
    logger.info(
      ...
      'constructor',
      'BlockchainAdapter initialization completed'
    );
  }
  ...
  async validateConfiguration() {
    ...
    if (!validation.overall.status === 'passed') {
      const error = new Error(
        `Configuration validation failed: ${validation.overall.error || 'Unknown error'}`
      );
  ...
  async initializeServices() {
    ...
    if (this.initializingServices) {
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'initialize_services',
        'Service initialization already in progress, waiting...'
      );
      ...
      if (suiInitialized) {
        ...
        this.suiService.subscribeToEvents((event) => {
          logger.info(
            LogComponent.BLOCKCHAIN_ADAPTER,
            'blockchain_event',
            `Blockchain event received: ${event.type}`,
            {
  ...
  resetTransactionState() {
    logger.info(
      ...
      'Manually resetting transaction state',
      {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'reset_transaction_state',
      'Transaction state reset successfully'
    );
  }
  ...
  getTransactionState() {
    ...
    return {
      ...
      timeSinceLastFailure: this.transactionState.lastFailureTime
        ? Date.now() - this.transactionState.lastFailureTime
        : null,
  ...
  handleBlockchainEvent(event) {
    switch (event?.type) {
      case 'SpreadsheetCreated':
        this.spreadsheetObjectId = event?.data?.spreadsheet_id ?? this.spreadsheetObjectId;
        break;
      case 'VersionSaved':
  ...
  async createNewSpreadsheetOptimized(title = 'Untitled Spreadsheet', initialData = null) {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'create_optimized_start',
      'Starting optimized spreadsheet creation',
      {
        title,
    ...
    if (!networkConfig?.packageId || !networkConfig?.registryObjectId) {
      ...
      return {
        success: false,
        error: 'Sui is not configured for the current network. Set packageId and registryObjectId.',
      };
    }
    ...
    try {
      ...
      return {
        success: false,
        error: `Configuration validation failed: ${typeof error === 'string' ? error : (error && error.message) || 'Unknown error'}`,
      };
    }
    ...
    if (!this.isWalletConnected()) {
      logger.warn(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'create_optimized_blocked',
        'Creation blocked - wallet not connected'
      );
      return {
        success: false,
    ...
    try {
      ...
      logger.info(
        LogComponent.STORAGE_SERVICE,
        'walrus_store_optimized',
        `Storing spreadsheet data in Walrus with ${defaultEpochs} epochs`
      );
      ...
      logger.info(
        ...
        'Creating single combined transaction for creation + version save'
      );
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'wallet_prompt_combined',
        'Two wallet prompts: Create spreadsheet + save initial version'
      );
      ...
      if (this.storageAdapter) {
        try {
          ...
          logger.info(
            LogComponent.BLOCKCHAIN_ADAPTER,
            'spreadsheet_persisted',
            'Spreadsheet ID persisted to storage',
            {
          ...
          logger.warn(
            LogComponent.BLOCKCHAIN_ADAPTER,
            'spreadsheet_persist_failed',
            'Failed to persist spreadsheet ID',
            {
              error: typeof e === 'string' ? e : e?.message,
            }
          );
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'two_transaction_success',
        'Two transactions executed successfully - spreadsheet created AND initial version saved!',
        {
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'create_optimized_completed',
        'ULTRA-OPTIMIZED spreadsheet creation completed with SINGLE transaction!',
        {
          duration,
      ...
      const moduleName =
        error?.moduleName ||
        (error?.stack?.match(/at ([^(]+)/) ? error.stack.match(/at ([^(]+)/)[1] : 'unknown');
      const stack = error?.stack || '';

      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'create_optimized_failed',
        'Optimized spreadsheet creation failed',
        {
          stage: errorStage,
          error: errorMessage,
          moduleName,
  ...
  async saveToBlockchainEnhanced(data, options = {}) {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'save_enhanced_start',
      `[${saveId}] Starting enhanced blockchain save`,
      {
        saveId,
    ...
    if (!this.isWalletConnected()) {
      logger.warn(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'save_enhanced_blocked',
        'Enhanced save blocked - wallet not connected'
      );
    ...
    try {
      ...
      if (enhancedOptions.useDeltaCompression && this.spreadsheetObjectId) {
        ...
        try {
          ...
          console.warn(
            'Could not retrieve previous version for delta compression, using standard storage'
          );
        }
      }
      ...
      if (enhancedOptions.useRedundancy || enhancedOptions.criticalData) {
        storageStrategy =
          storageStrategy === 'delta-compression' ? 'delta-with-redundancy' : 'redundancy';
      }
      ...
      switch (storageStrategy) {
        ...
        case 'redundancy':
          console.log(`Storing with ${enhancedOptions.redundancyLevel}x redundancy`);
        ...
        case 'delta-with-redundancy': {
          console.log(
            `Storing with delta compression AND ${enhancedOptions.redundancyLevel}x redundancy`
          );
          ...
          if (
            deltaResult.success &&
            deltaResult.compressionInfo?.compressionRatio >= enhancedOptions.compressionThreshold
          ) {
            ...
            console.log('Delta compression not effective, using redundancy only');
            walrusResult = await this.walrusService.storeWithRedundancy(
              data,
              enhancedOptions.redundancyLevel,
              { spreadsheetId: this.spreadsheetObjectId || data.title, chunk: options.chunk }
      ...
      if (!this.spreadsheetObjectId) {
        ...
        if (!executeResult.success) {
          throw new Error(
            `Enhanced blockchain transaction failed: ${executeResult.error || 'Unknown error'}`
          );
        }
      ...
      const transactionDigest =
        ...
        executeResult.saveTransactionDigest ||
        executeResult.createTransactionDigest;
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'save_enhanced_success',
        'Enhanced blockchain save completed',
        {
          saveId,
      ...
      logger.error(
        ...
        'Enhanced blockchain save failed',
        {
          saveId,
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
  ...
  async saveToBlockchain(data, options = {}) {
    ...
    if (!this.walrusService) {
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'preflight_failed',
        'Walrus service not initialized'
      );
      return {
        success: false,
    ...
    if (atomicResult.success) {
      ...
      if (walrusResult && blockchainResult) {
        ...
        logger.info(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'save_completed',
          'Parallel save operation completed successfully',
          {
        ...
        logger.warn(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'save_incomplete',
          'Parallel operations completed but missing expected results',
          {
  ...
  _normalizeSpreadsheetData(input) {
    ...
    const title =
      top.title || nested.metadata?.title || top.metadata?.title || 'Untitled Spreadsheet';
  ...
  async saveWithProgressiveEnhancement(data, options = {}) {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'progressive_save_start',
      'Starting progressive save',
      {
    ...
    const fallbackSave = async () => {
      logger.warn(
        LogComponent.STORAGE_SERVICE,
        'fallback_save_local',
        'Using local storage fallback - blockchain unavailable'
      );
  ...
  async createSpreadsheetWithProgressiveEnhancement(title = 'Untitled Spreadsheet') {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'progressive_create_start',
      'Starting progressive create',
      {
        title,
        walletConnected: this.isWalletConnected(),
      }
    ...
    const fallbackCreate = async () => {
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'fallback_create_local',
        'Creating local-only spreadsheet'
      );
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'progressive_create_complete',
      'Progressive create completed',
      {
        success: result.success,
        method: result.method,
        fallback: result.fallback,
  ...
  async createOfflineSpreadsheet(title = 'Untitled Spreadsheet') {
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'create_offline_start',
      'Creating offline spreadsheet',
      { title }
    );
  ...
  async syncOfflineData() {
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'sync_offline_start',
      'Starting offline data synchronization'
    );
  ...
  async createSpreadsheetSmart(title = 'Untitled Spreadsheet') {
    ...
    if (shouldUseOffline) {
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'smart_create_offline',
        'Using offline mode for creation'
      );
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'smart_create_online',
        'Using online mode for creation'
      );
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'smart_create_complete',
      'Smart create completed',
      {
        success: result.success,
  ...
  queueForLaterSync(data) {
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'queue_data',
      'Queueing data for later blockchain sync',
      {
  ...
  async getUserSpreadsheets() {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'spreadsheet_discovery',
      'Fetching user spreadsheets from blockchain'
    );

    if (!this.isWalletConnected()) {
      logger.warn(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_discovery_blocked',
        'Cannot fetch spreadsheets - wallet not connected'
      );
      return {
        success: false,
        error: 'Wallet not connected',
    ...
    try {
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_discovery_success',
        'Successfully fetched user spreadsheets',
        {
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_discovery_failed',
        'Failed to fetch user spreadsheets',
        {
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
      );
  ...
  async loadSpreadsheetEnhanced(spreadsheetId, onProgress = null) {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'spreadsheet_load_enhanced',
      `[${loadId}] Loading spreadsheet with enhanced features`,
      {
        loadId,
        spreadsheetId,
    ...
    if (!this.isWalletConnected()) {
      logger.warn(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_load_enhanced_blocked',
        'Cannot load spreadsheet - wallet not connected'
      );
      return { success: false, error: 'Wallet not connected' };
    }
    ...
    try {
      ...
      console.log(`[${loadId}] Fetching enhanced spreadsheet data`, {
        spreadsheetId,
        timestamp: new Date().toISOString(),
      });
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_load_enhanced_success',
        'Enhanced spreadsheet load completed',
        {
          spreadsheetId,
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_load_enhanced_failed',
        'Enhanced spreadsheet load failed',
        {
          spreadsheetId,
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
  ...
  async loadSpreadsheet(spreadsheetId, onProgress = null) {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'spreadsheet_load',
      `[${loadId}] Loading spreadsheet from blockchain`,
      {
        loadId,
        spreadsheetId,
        hasProgress: !!onProgress,
    ...
    try {
      ...
      const fetchStartTime = Date.now();
      const result = await this.suiService.getSpreadsheetData(
        spreadsheetId,
        this.walrusService,
        onProgress
      );
      ...
      try {
        ...
        if (!versionCompatibility.compatible) {
          ...
          const isLegacySpreadsheet =
            versionCompatibility.spreadsheetVersion === 0 &&
            versionCompatibility.moduleVersion === 1;
          ...
          'Failed to check version compatibility',
          {
            error:
              typeof versionError === 'string'
                ? versionError
                : (versionError && versionError.message) || 'Unknown error',
          }
        );
      ...
      if (result.isEmptySpreadsheet) {
        logger.info(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_load_empty',
          'Loaded empty spreadsheet (no versions)',
          {
            spreadsheetId,
            duration,
          }
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_load_success',
        'Successfully loaded spreadsheet',
        {
          spreadsheetId,
          version: result.version.version_number,
          cellCount: result.version.cell_count,
  ...
  async verifySpreadsheetIntegrity(options = {}) {
    try {
      ...
      const versions = await this.suiService.getEnhancedSpreadsheetVersions(
        this.spreadsheetObjectId
      );
      if (versions.length === 0) {
        return {
          success: true,
  ...
  async renameSpreadsheet(spreadsheetId, newTitle) {
    ...
    if (!this.isWalletConnected()) {
      logger.warn(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_rename_blocked',
        'Cannot rename spreadsheet - wallet not connected'
      );
      return {
        success: false,
        error: 'Wallet not connected',
    ...
    try {
      ...
      if (result.success) {
        logger.info(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_rename_success',
          'Successfully renamed spreadsheet',
          {
            spreadsheetId,
            newTitle,
            duration,
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_rename_error',
        'Error renaming spreadsheet',
        {
          spreadsheetId,
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
  ...
  async makeSpreadsheetPublic(spreadsheetId) {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'spreadsheet_make_public',
      'Making spreadsheet public',
      {
        spreadsheetId,
      }
    );
    ...
    if (!this.isWalletConnected()) {
      logger.warn(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_public_blocked',
        'Cannot make public - wallet not connected'
      );
      return {
        success: false,
        error: 'Wallet not connected',
    ...
    try {
      ...
      if (result.success) {
        logger.info(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_public_success',
          'Successfully made spreadsheet public',
          {
            spreadsheetId,
            duration,
            transactionDigest: result.transactionDigest,
        ...
        logger.warn(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_public_failed',
          'Failed to make spreadsheet public',
          {
            spreadsheetId,
            error: result.error,
          }
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_public_error',
        'Error making spreadsheet public',
        {
          spreadsheetId,
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
  ...
  async makeSpreadsheetPrivate(spreadsheetId) {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'spreadsheet_make_private',
      'Making spreadsheet private',
      {
        spreadsheetId,
      }
    );
    ...
    if (!this.isWalletConnected()) {
      logger.warn(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_private_blocked',
        'Cannot make private - wallet not connected'
      );
      return {
        success: false,
        error: 'Wallet not connected',
    ...
    try {
      ...
      if (result.success) {
        logger.info(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_private_success',
          'Successfully made spreadsheet private',
          {
            spreadsheetId,
            duration,
            transactionDigest: result.transactionDigest,
        ...
        logger.warn(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_private_failed',
          'Failed to make spreadsheet private',
          {
            spreadsheetId,
            error: result.error,
          }
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_private_error',
        'Error making spreadsheet private',
        {
          spreadsheetId,
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
  ...
  async transferSpreadsheetOwnership(spreadsheetId, newOwnerAddress) {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'spreadsheet_transfer',
      'Transferring spreadsheet ownership',
      {
        spreadsheetId,
    ...
    if (!this.isWalletConnected()) {
      logger.warn(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_transfer_blocked',
        'Cannot transfer ownership - wallet not connected'
      );
      return {
        success: false,
        error: 'Wallet not connected',
    ...
    try {
      const result = await this.suiService.transferSpreadsheetOwnership(
        spreadsheetId,
        newOwnerAddress
      );
      ...
      if (result.success) {
        logger.info(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_transfer_success',
          'Successfully transferred spreadsheet ownership',
          {
            spreadsheetId,
        ...
        logger.warn(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_transfer_failed',
          'Failed to transfer spreadsheet ownership',
          {
            spreadsheetId,
            error: result.error,
          }
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_transfer_error',
        'Error transferring spreadsheet ownership',
        {
          spreadsheetId,
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
  ...
  async pruneSpreadsheetVersions(spreadsheetId, keepCount = 10) {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'spreadsheet_prune',
      'Pruning old spreadsheet versions',
      {
        spreadsheetId,
        keepCount,
      }
    ...
    if (!this.isWalletConnected()) {
      logger.warn(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_prune_blocked',
        'Cannot prune versions - wallet not connected'
      );
      return {
        success: false,
        error: 'Wallet not connected',
    ...
    try {
      ...
      if (result.success) {
        logger.info(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_prune_success',
          'Successfully pruned old versions',
          {
            spreadsheetId,
        ...
        logger.warn(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_prune_failed',
          'Failed to prune old versions',
          {
            spreadsheetId,
            error: result.error,
          }
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_prune_error',
        'Error pruning old versions',
        {
          spreadsheetId,
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
  ...
  async deleteSpreadsheet(spreadsheetId) {
    ...
    logger.warn(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'spreadsheet_delete',
      'DELETING SPREADSHEET PERMANENTLY',
      {
    ...
    if (!this.isWalletConnected()) {
      logger.warn(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'spreadsheet_delete_blocked',
        'Cannot delete spreadsheet - wallet not connected'
      );
      return {
        success: false,
        error: 'Wallet not connected',
    ...
    try {
      ...
      if (result.success) {
        logger.warn(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_delete_success',
          'Successfully deleted spreadsheet',
          {
            spreadsheetId,
        ...
        if (this.spreadsheetObjectId === spreadsheetId) {
          logger.info(
            LogComponent.BLOCKCHAIN_ADAPTER,
            'clear_current_spreadsheet',
            'Clearing current spreadsheet from local state'
          );
          ...
          if (this.storageAdapter) {
            try {
              ...
              logger.info(
                LogComponent.BLOCKCHAIN_ADAPTER,
                'session_cleared',
                'Session cleared for deleted spreadsheet'
              );
            } catch (clearError) {
              logger.warn(
                LogComponent.BLOCKCHAIN_ADAPTER,
          ...
          logger.info(
            LogComponent.BLOCKCHAIN_ADAPTER,
            'sync_status_reset',
            'Sync status reset after deletion'
          );
        }
      } else {
        logger.error(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'spreadsheet_delete_failed',
  ...
  async createTransactionWithEstimation(operation, params = {}) {
    ...
    logger.info(
      ...
      'Creating transaction with estimation',
      {
        operation,
        params,
      }
    ...
    try {
      ...
      switch (operation) {
        case 'create_spreadsheet':
          transaction = await this.suiService.createSpreadsheetTransaction(
            params.title || 'Untitled Spreadsheet'
          );
        ...
        case 'update_title':
          transaction = this.suiService.createUpdateTitleTransaction(
        ...
        case 'transfer_ownership':
          transaction = this.suiService.createTransferOwnershipTransaction(
            params.spreadsheetId,
            params.newOwner
          );
      ...
      logger.info(
        ...
        'Transaction created with estimation',
        {
          operation,
  ...
  async executeTransactionWithRecovery(transaction, context = {}) {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'transaction_execute_start',
      'Executing transaction with recovery',
      context
    );
  ...
  clearInvalidObjectCache() {
    logger.warn(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'cache_cleanup',
      'Clearing invalid object cache in BlockchainAdapter'
    );
    ...
    try {
      ...
      if (this.spreadsheetObjectId) {
        logger.warn(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'cache_clear_object_id',
          'Clearing cached spreadsheet object ID',
          {
      ...
      if (this.syncQueue && this.syncQueue.length > 0) {
        logger.warn(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'cache_clear_sync_queue',
          'Clearing sync queue due to invalid objects',
          {
            queueSize: this.syncQueue.length,
          }
        );
      ...
      if (this.storageAdapter) {
        if (typeof this.storageAdapter.setCurrentSpreadsheetId === 'function') {
          ...
          logger.info(
            LogComponent.BLOCKCHAIN_ADAPTER,
            'cache_clear_session_id',
            'Cleared current spreadsheet ID from storage adapter'
          );
        }
        ...
        if (typeof this.storageAdapter.clearObjectCache === 'function') {
          ...
          logger.info(
            LogComponent.BLOCKCHAIN_ADAPTER,
            'cache_clear_storage_cache',
            'Cleared storage adapter object cache'
          );
        }
      }
      ...
      logger.info(
        ...
        'BlockchainAdapter cache cleared'
      );
    } catch (error) {
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'cache_cleanup_error',
        'Error clearing BlockchainAdapter cache',
        {
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
      );
  ...
  cleanup() {
    ...
    try {
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'cleanup_complete',
        'BlockchainAdapter cleanup completed'
      );
    } catch (error) {
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'cleanup_error',
        'Error during BlockchainAdapter cleanup',
        {
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
          stack: error.stack,
        }
  ...
  async retryFallbackSave(localKey) {
    try {
      ...
      if (!fallbackData) {
        logger.warn(LogComponent.BLOCKCHAIN_ADAPTER, 'retry_not_found', 'Fallback key not found', {
          localKey,
        });
        return {
          success: false,
      ...
      if (saveResult.success) {
        ...
        if (typeof window !== 'undefined') {
          window.dispatchEvent(
            new CustomEvent('save:retry-success', {
              detail: {
                localKey,
                timestamp: Date.now(),
              },
        ...
        if (typeof window !== 'undefined') {
          window.dispatchEvent(
            new CustomEvent('save:retry-failed', {
              detail: {
                localKey,
  ...
  async autoRetryFallbacksOnStartup() {
    try {
      ...
      if (!this.isWalletConnected()) {
        logger.warn(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'startup_retry_no_wallet',
          'Wallet not connected - skipping auto-retry'
        );
        return;
      }
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'startup_retry_start',
        'Starting auto-retry of fallback saves',
        {
      ...
      logger.info(
        ...
        'Auto-retry of fallback saves complete',
        {
          successCount,
          failureCount,
      ...
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('save:startup-retry-complete', {
            detail: {
              successCount,
              failureCount,
              total: fallbackKeys.length,
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'startup_retry_error',
        'Error during startup auto-retry',
        {
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
      );
  ...
  checkStaleFallbacks(maxAgeDays = 7) {
    try {
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'stale_check_error',
        'Error checking stale fallbacks',
        {
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
      );
    ...
    try {
      ...
      for (const key of keys) {
        try {
          ...
          logger.debug(LogComponent.BLOCKCHAIN_ADAPTER, 'stale_deleted', 'Deleted stale fallback', {
            key,
          });
        } catch (e) {
          ...
          logger.warn(
            LogComponent.BLOCKCHAIN_ADAPTER,
            'stale_delete_error',
            'Failed to delete stale fallback',
            {
              key,
              error: e.message,
            }
      ...
      logger.info(
        ...
        'Stale fallback cleanup complete',
        {
          deletedCount,
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'stale_cleanup_error',
        'Error during stale fallback cleanup',
        {
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
      );
    ...
    try {
      ...
      if (!data) {
        logger.warn(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'export_not_found',
          'Fallback key not found for export',
          { key }
        );
        return null;
      }
      ...
      logger.info(LogComponent.BLOCKCHAIN_ADAPTER, 'export_complete', 'Exported fallback as JSON', {
        key,
      });
    ...
    if (typeof window !== 'undefined') {
      ...
      window.addEventListener('save:retry-fallbacks', async (event) => {
        ...
        logger.info(
          ...
          'Manual fallback retry triggered',
          {
            count: fallbackKeys.length,
          }
        );
      ...
      logger.debug(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'retry_listeners_setup',
        'Fallback retry event listeners setup'
      );
    }
  }
  ...
  async retryBlockchainCommit() {
    ...
    logger.info(
      LogComponent.BLOCKCHAIN_ADAPTER,
      'retry_blockchain_start',
      'Starting blockchain commit retry (no Walrus re-upload)'
    );
    ...
    try {
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'retry_blockchain_pending_data',
        'Retrieved pending blockchain data',
        {
      ...
      logger.info(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'retry_blockchain_transaction_created',
        'Storage transaction created for retry'
      );
      ...
      if (result.success) {
        ...
        logger.info(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'retry_blockchain_success',
          'Blockchain commit retry succeeded',
          {
            transactionDigest: result.digest,
            duration,
            blobId: pendingData.walrusBlobId,
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'retry_blockchain_failed',
        'Blockchain commit retry failed',
        {
          error: typeof error === 'string' ? error : (error && error.message) || 'Unknown error',
        }
      );
    ...
    try {
      ...
      if (this._cachedHealthCheck && now - this._cachedHealthCheck.timestamp < 30000) {
        logger.debug(
          LogComponent.BLOCKCHAIN_ADAPTER,
          'health_check_cached',
          'Using cached Walrus health check',
          {
      ...
      logger.error(
        LogComponent.BLOCKCHAIN_ADAPTER,
        'health_check_failed',
        'Walrus health check threw exception',
        {
          error: error.message,
          stack: error.stack,
        }
...
Current state:`,
        window.WalSheetzDebug.getTransactionState()
      );
    },
  };
