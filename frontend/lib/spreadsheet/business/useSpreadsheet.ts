...
import { useWalletConnectionFactory } from '@shared/hooks/useWalletConnectionFactory';
...
export function useSpreadsheet() {
  ...
  const [_spreadsheetCount, _setSpreadsheetCount] = useState(0);
  ...
  const _webSocketRef = useRef(null);
  ...
  const autoDiscoverSpreadsheets = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.spreadsheets.length > 0) {
        ...
        _setSpreadsheetCount(result.spreadsheets.length);

        // Optionally auto-load the most recent spreadsheet
        const mostRecent = result.spreadsheets[0];
        if (mostRecent) {
          console.log(
            `Most recent spreadsheet: "${mostRecent.title}" (${new Date(mostRecent.last_modified).toLocaleDateString()})`
          );
        }
      }
    } catch (error) {
      console.warn(
        'Failed to auto-discover spreadsheets:',
  ...
  const checkSessionRestoration = useCallback(async () => {
    ...
    if (loadingOperationRef.current) {
      console.log('Load operation in progress, skipping session restoration');
      return;
    }
    ...
    if (sessionInfo.hasSpreadsheet && sessionInfo.hasWalletAddress) {
      ...
      if (storedWallet === walletConnection.address) {
        console.log('WalSheetz: Attempting to restore previous session...', {
          ...
          lastSave: sessionInfo.lastSaveTimestamp
            ? new Date(sessionInfo.lastSaveTimestamp).toLocaleString()
            : 'Unknown',
        });
        ...
        try {
          if (spreadsheetId && blockchainRef.current && engineRef.current) {
            ...
            const restorationTimeout = setTimeout(() => {
              console.warn('Session restoration taking too long, clearing state');
            ...
            try {
              ...
              if (
                ...
                typeof blockchainRef.current.loadSpreadsheet !== 'function'
              ) {
              ...
              for (let attempt = 0; attempt <= sessionGuards.maxRetries; attempt++) {
                try {
                  if (attempt > 0) {
                    console.log(
                      `Session restoration retry ${attempt}/${sessionGuards.maxRetries} after ${sessionGuards.retryDelay}ms`
                    );
                    await new Promise((resolve) =>
                      setTimeout(resolve, sessionGuards.retryDelay * attempt)
                    );
                  ...
                  onProgress(
                    `Loading spreadsheet (attempt ${attempt + 1}/${sessionGuards.maxRetries + 1})...`,
                  ...
                  if (result && result.success) {
                    ...
                    console.warn(
                      `Session restoration attempt ${attempt + 1} failed:`,
                      result.error
                    );
                  }
                } catch (attemptError) {
                  ...
                  console.warn(
                    `Session restoration attempt ${attempt + 1} error:`,
                    attemptError.message
                  );

                  // Don't retry on certain critical errors
                  if (
                    attemptError.message.includes('Wallet disconnected') ||
              ...
              if (result.metadata?.isEmpty) {
                console.warn('Session restoration: chain empty; keeping restored/local data');
                try {
                ...
                try {
                  ...
                  console.warn(
                    'Engine data loading failed during session restoration:',
                    engineError.message
                  );
              ...
              try {
                ...
                console.warn('Session metadata update failed:', metaError.message);
              ...
              try {
                if (blockchainRef.current) {
                  setSyncStatus(blockchainRef.current.getSyncStatus());
                }
              } catch (syncError) {
                console.warn('Sync status update failed during restoration:', syncError.message);
                // Continue without sync status update
              }

              console.log('WalSheetz: Session restored successfully');
              setLoadingState({ isLoading: false, message: '', details: '' });
              setSaveStatus('saved');
              ...
              clearTimeout(restorationTimeout);
              console.warn(
                'WalSheetz: Error during session restoration:',
          ...
          console.warn(
            'WalSheetz: Failed to restore session:',
            typeof error === 'string' ? error : error.message || 'Unknown error'
          );
          storageRef.current.clearSession();
          setLoadingState({ isLoading: false, message: '', details: '' });
  ...
  useEffect(() => {
    ...
    walletConnection.isConnected,
    walletConnection.isAutoConnecting,
  ...
  useEffect(() => {
    const initializeServices = async () => {
      ...
      const initializationTimeout = setTimeout(() => {
        console.warn('Initialization taking too long (>10s), clearing loading state');
        setLoadingState({ isLoading: false, message: '', details: '' });
        setSaveStatus('error');
      ...
      try {
        ...
        console.log('Creating BlockchainAdapter...');
        ...
        if (result.success) {
          ...
          if (result.data) {
            ...
            try {
              ...
              // Luckysheet may not be ready yet
            }
            setSpreadsheetData(result.data);
            // Ensure refresh after Luckysheet finishes init
            setTimeout(() => {
              try {
                engineRef.current?.loadData?.(result.data);
              } catch {
        ...
        try {
          ...
          if (
            sig.debug?.reason === 'abi_missing_function' ||
            sig.debug?.reason === 'detection_error'
          ) {
            console.warn(
              '[useSpreadsheet] ABI detection used fallback - ensure config flags match deployed contract'
            );
          ...
          console.warn(
            '[useSpreadsheet] This may indicate RPC connectivity issues - transactions may fail if config is incorrect'
          );
        }
  ...
  const _updateWalletStatus = useCallback(() => {
  ...
  const handleCellEdit = useCallback(
    (row, col, oldValue, newValue) => {
      ...
      if (row === null || row === undefined) {
        console.error('useSpreadsheet.handleCellEdit: row is null or undefined', {
          row,
          col,
          oldValue,
          newValue,
      ...
      if (col === null || col === undefined) {
        console.error('useSpreadsheet.handleCellEdit: col is null or undefined', {
          row,
          col,
          oldValue,
          newValue,
      ...
      if (typeof row !== 'number' || row < 0) {
        console.error('useSpreadsheet.handleCellEdit: invalid row value', {
          row,
          col,
          oldValue,
          newValue,
      ...
      if (typeof col !== 'number' || col < 0) {
        console.error('useSpreadsheet.handleCellEdit: invalid col value', {
          row,
          col,
          oldValue,
          newValue,
  ...
  const setWalrusEpochPreference = useCallback(async (epochs) => {
    ...
    try {
      ...
      if (success) {
        console.log(
          `[useSpreadsheet] Walrus epoch preference saved for spreadsheet: ${epochs} epochs`
        );
        return { success: true };
      } else {
  ...
  const saveToBlockchain = useCallback(
    async (title = null, epochs = null) => {
      ...
      if (config?.isFallback) {
        logger.warn(
          LogComponent.BUSINESS_LOGIC,
          'fallback_config_in_use',
          'Using fallback config - endpoints may be stale'
        );
        ...
        if (missingFields.length > 0) {
          ...
          logger.error(
            ...
            'Fallback config missing critical fields',
            {
              missingFields,
      ...
      if (isFirstSave) {
        ...
        try {
          ...
          logger.info(
            LogComponent.BUSINESS_LOGIC,
            'first_save_preflight',
            'Checking Walrus health before first save'
          );
          logger.info(
            LogComponent.BUSINESS_LOGIC,
            'telemetry:stage',
            'Starting walrus_store stage for first save'
          );

          const walrusHealth = await blockchainRef.current.checkWalrusHealth?.();
          if (walrusHealth && !walrusHealth.ok) {
            const _errorMsg = walrusHealth.error || 'Walrus storage unavailable';

            // Log degraded mode but continue with single attempt instead of aborting
            logger.warn(
              LogComponent.BUSINESS_LOGIC,
              'walrus_degraded_mode',
          ...
          logger.info(
            LogComponent.BUSINESS_LOGIC,
            'walrus_health_check_complete',
            'Walrus health check complete, proceeding with first save'
          );
          ...
          try {
            ...
            console.log(
              '[First Save] Data collected successfully, size:',
            ...
            throw new Error(
              `Failed to collect spreadsheet data: ${dataCollectionError?.message || String(dataCollectionError)}`
            );
          }
          ...
          if (result.success) {
            ...
            logger.info(
              LogComponent.BUSINESS_LOGIC,
              'telemetry:stage',
              'create_tx and save_tx completed',
              {
            ...
            try {
              if (blockchainRef.current?.suiService?.getLatestVersionMetadata) {
                const versionMetadata =
                  await blockchainRef.current.suiService.getLatestVersionMetadata(
                ...
                if (versionMetadata && storageRef.current?.setVersionMetadata) {
                  ...
                  logger.info(
                    LogComponent.BUSINESS_LOGIC,
                    'version_metadata_persisted',
                    'Version metadata saved for Save Details modal',
                    {
              ...
              logger.warn(
                LogComponent.BUSINESS_LOGIC,
                'version_metadata_failed',
                'Could not fetch version metadata',
                {
          ...
          logger.error(
            LogComponent.BUSINESS_LOGIC,
            'first_save_exception',
            'Exception during first save',
            {
              error: errorMessage,
              stack: error?.stack,
      ...
      try {
        ...
        try {
          ...
          if (spreadsheetId && blockchainRef.current?.suiService?.validateSpreadsheetObjectExists) {
            const existsCheck =
              await blockchainRef.current.suiService.validateSpreadsheetObjectExists(spreadsheetId);
            if (!existsCheck.exists) {
              console.warn(
                'Pre-save: stale spreadsheetId detected, clearing session and forcing create path',
                existsCheck
              );
        ...
        if (error.message?.includes('Wallet communication failed')) {
          userMessage =
            'Wallet connection issue detected. Please refresh the page and reconnect your wallet.';
          ...
          userMessage =
            'Insufficient SUI balance. Please add funds to your wallet or use the faucet.';
        }
  ...
  const getUserSpreadsheets = useCallback(async () => {
    if (!blockchainRef.current)
      return { success: false, error: 'Blockchain adapter not initialized' };
  ...
  const loadSpreadsheet = useCallback(
    async (spreadsheetId) => {
      ...
      if (loadingOperationRef.current === spreadsheetId) {
        console.log(`Already loading spreadsheet ${spreadsheetId}, skipping duplicate request`);
      ...
      if (loadingOperationRef.current) {
        console.log(
          `Another spreadsheet (${loadingOperationRef.current}) is loading, queuing current load of ${spreadsheetId}`
        );
      ...
      try {
        ...
        for (let attempt = 0; attempt <= WALRUS_MAX_RETRIES; attempt++) {
          try {
            if (attempt > 0) {
              ...
              console.log(
                `Walrus fetch retry ${attempt}/${WALRUS_MAX_RETRIES} after ${delayMs}ms delay`
              );
            ...
            if (result && result.success) {
              ...
              if (
                (result.error.includes('Walrus') ||
                  result.error.includes('fetch') ||
                  result.error.includes('network')) &&
                attempt < WALRUS_MAX_RETRIES
              ) {
                console.warn(
                  `Walrus-related error on attempt ${attempt + 1}, will retry: ${result.error}`
                );
                continue; // Retry
              } else {
            ...
            if (isRetryableError && attempt < WALRUS_MAX_RETRIES) {
              console.warn(
                `Walrus fetch failed (attempt ${attempt + 1}), will retry: ${attemptError.message}`
              );
              continue; // Retry
            } else if (!isRetryableError) {
              console.warn(
                `Non-retryable error detected, skipping retries: ${attemptError.message}`
              );
              throw attemptError; // Don't retry non-retryable errors
            } else {
              console.warn(`Max retries reached (${WALRUS_MAX_RETRIES}), giving up`);
        ...
        if (result.success) {
          ...
          if (result.metadata?.isEmpty) {
            console.warn(
              '[useSpreadsheet] Chain returned empty spreadsheet (no versions); clearing grid and loading empty data'
            );
          ...
          if (result.error === 'Wallet not connected') {
            ...
            errorMessage =
              'Failed to retrieve data from Walrus storage. The blob may have expired.';
  ...
  const transferOwnership = useCallback(async (spreadsheetId, newOwnerAddress) => {
    ...
    setSaveStatus('saving');
    const result = await blockchainRef.current.transferSpreadsheetOwnership(
      spreadsheetId,
      newOwnerAddress
    );
    setSaveStatus('ready');
  ...
  const createNewSpreadsheet = useCallback(
    async (title = 'Untitled Spreadsheet') => {
      ...
      try {
        ...
        if (error.message?.includes('Wallet communication failed')) {
          userMessage =
            'Wallet connection issue detected. Please refresh the page and reconnect your wallet before creating a new spreadsheet.';
        } else if (error.message?.includes('not connected')) {
          userMessage =
            'Wallet not connected. Please connect your Slush wallet before creating a spreadsheet.';
        } else if (error.message?.includes('Low wallet balance')) {
          userMessage =
            'Insufficient SUI balance. Please add funds to your wallet or use the faucet before creating a spreadsheet.';
        }
  ...
  const deleteSpreadsheet = useCallback(
    async (spreadsheetId, title) => {
      ...
      try {
        console.log('[useSpreadsheet] Starting spreadsheet deletion:', { spreadsheetId, title });
