...
import { faucetService } from '@lib/spreadsheet/services/FaucetService';
...
export function StatusBar({
  ...
  const handleRequestTestSui = async () => {
    ...
    try {
      ...
      if (result.success) {
        setFaucetMessage(`OK: ${result.message}`);
        ...
        setFaucetMessage(`Error: ${result.error}`);

        // Clear error message after 8 seconds
        setTimeout(() => setFaucetMessage(''), 8000);
      }
    } catch (error) {
      setFaucetMessage(`Error: Network error: ${error.message}`);
      setTimeout(() => setFaucetMessage(''), 8000);
    } finally {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            ...
            {walletConnected && (
              <div style={{ fontSize: '10px', color: '#6b7280', display: 'flex', gap: '8px' }}>
                {walletAddress && <span title={walletAddress}>{formatAddress(walletAddress)}</span>}
                {walletBalance && <span>{formatBalance(walletBalance)}</span>}
      ...
      {faucetMessage && (
        <div className="status-section">
          <span
            ...
            style={{
              fontSize: '11px',
              color: faucetMessage.startsWith('OK:') ? '#059669' : '#dc2626',
              fontWeight: '500',
            }}
          >
            {faucetMessage}
